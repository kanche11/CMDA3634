...
scene_t *sceneSetup(){
  ...
  scene-> c_materials = cudaMalloc(*materials,Nmaterials*sizeof(material_t));
  scene->c_materials = cudaMemcpy(*materials,*c_materials,Nmaterials,cudaMemcpyHostToDevice);
  scene-> c_lights = cudaMalloc(*lights,Nlights*sizeof(light_t));
  scene->c_lights = cudaMemcpy(*lights,*c_lights,Nlights,cudaMemcpyHostToDevice);
  scene-> c_grid = cudaMalloc(grid,sizeof(grid_t));
  scene->c_grid = cudaMemcpy(grid,c_grid,1,cudaMemcpyHostToDevice);
  return scene;
}
