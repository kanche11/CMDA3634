...
void gridPopulate(grid_t *grid, int Nshapes, shape_t *shapes){
  ...
  scene-> c_boxContents = cudaMalloc(*boxContents,Nentries*sizeof(int));
  scene->c_boxContents = cudaMemcpy(*boxContents,*c_boxContents,Nentries,cudaMemcpyHostToDevice);
  scene-> c_boxStarts = cudaMalloc(*boxStarts,(Nboxes+1)*sizeof(int));
  scene->c_boxStarts = cudaMemcpy(*boxStarts,*c_boxStarts,Nboxes+1,cudaMemcpyHostToDevice);
